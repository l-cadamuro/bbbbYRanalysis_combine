...
hdata = get_histo(fIn, 'data_obs', selection, final_var)
...
writelnarr (fcard, ['rate']
                + ["%.0f" % get_histo(fIn, hname, selection, final_var).Integral() for hname in sigs]
                + ["%.0f" % get_histo(fIn, hname, selection, final_var).Integral() for hname in bkgs], addEmptyAtIdx=1)
writeln    (fcard, '----------------------------------------------------------------------------------------------------------------------------------')
